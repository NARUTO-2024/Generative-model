...
class LinearAttention(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        qkv = self.to_qkv(x)
        q, k, v = rearrange(qkv, 'b (qkv heads c) h w -> qkv b heads c (h w)', heads = self.heads, qkv=3)  # 将输入的 qkv 张量进行重排（rearrange），提取出查询 (q)、键 (k) 和值 (v) 的张量
        k = k.softmax(dim=-1)
        context = torch.einsum('bhdn,bhen->bhde', k, v)  # 使用爱因斯坦求和约定，计算上下文向量
