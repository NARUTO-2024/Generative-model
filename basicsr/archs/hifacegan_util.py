...
class SPADE(nn.Module):
    def __init__(self, config_text, norm_nc, label_nc):
        ...
        param_free_norm_type = str(parsed.group(1))  # 提取参数无关的归一化类型
        ks = int(parsed.group(2))  # 提取卷积核的大小

        # 根据选择的归一化类型初始化参数无关的归一化层
        if param_free_norm_type == 'instance':
            self.param_free_norm = nn.InstanceNorm2d(norm_nc)  # 实例归一化
        elif param_free_norm_type == 'syncbatch':
            print('SyncBatchNorm is currently not supported under single-GPU mode, switch to "instance" instead')
            self.param_free_norm = nn.InstanceNorm2d(norm_nc)  # 使用实例归一化
        elif param_free_norm_type == 'batch':
            self.param_free_norm = nn.BatchNorm2d(norm_nc, affine=False)  # 批归一化，关闭仿射变换
        else:
            raise ValueError(f'{param_free_norm_type} is not a recognized param-free norm type in SPADE')

        # 中间嵌入空间的维度，基于输入特征通道数决定
        nhidden = 128 if norm_nc > 128 else norm_nc  # 如果输入通道数大于128，隐藏维度为128，否则为norm_nc

        pw = ks // 2  # 卷积核的 padding 大小
        # 定义一个共享的多层感知机（MLP）以处理语义分割图
        self.mlp_shared = nn.Sequential(
            nn.Conv2d(label_nc, nhidden, kernel_size=ks, padding=pw),  # 输入为语义图，输出为隐藏层
            nn.ReLU()  # 激活函数
        )
        # 定义用于计算缩放和偏移的 MLP
        self.mlp_gamma = nn.Conv2d(nhidden, norm_nc, kernel_size=ks, padding=pw, bias=False)  # 计算 gamma
        self.mlp_beta = nn.Conv2d(nhidden, norm_nc, kernel_size=ks, padding=pw, bias=False)  # 计算 beta

    def forward(self, x, segmap):
        # 前向传播
        # Part 1. 生成无参数归一化的激活
        normalized = self.param_free_norm(x)  # 对输入 x 进行归一化处理

        # Part 2. 生成基于语义图的缩放和偏移
        segmap = F.interpolate(segmap, size=x.size()[2:], mode='nearest')  # 上采样语义图，匹配输入特征图的尺寸
        actv = self.mlp_shared(segmap)  # 通过共享 MLP 处理语义图
        gamma = self.mlp_gamma(actv)  # 计算 gamma
        beta = self.mlp_beta(actv)  # 计算 beta

        # 应用缩放和偏移
        out = normalized * gamma + beta  # 进行归一化输出调整：归一化特征 * gamma + beta
