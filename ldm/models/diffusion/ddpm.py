...
class LatentDiffusion(DDPM):
    ...
    def get_fold_unfold(self, x, kernel_size, stride, uf=1, df=1):
        """
        根据输入图像张量 x 及指定的 kernel_size 和 stride，返回用于将图像块展开（unfold）和折叠（fold）的操作器，
        以及用于归一化重叠区域的加权系数和归一化张量。支持上采样（uf > 1）和下采样（df > 1）。

        :param x: 输入图像张量，形状为 (bs, c, h, w) (batch size, channel, height, width)
        :param kernel_size: 卷积核大小，形如 (kh, kw)
        :param stride: 步幅，形如 (sh, sw)
        :param uf: 上采样因子，默认值为 1（不做上采样）
        :param df: 下采样因子，默认值为 1（不做下采样）
        :return: 返回的值包括：
            - fold: 用于将图像块重新折叠成完整图像的操作器
            - unfold: 用于将图像展开成多个小块的操作器
            - normalization: 用于规范化重叠部分的张量
            - weighting: 归一化权重，用于图像块的权重处理
        """

        # 获取输入张量的形状，bs = batch size, nc = number of channels, h = height, w = width
        bs, nc, h, w = x.shape

        # 计算给定 kernel_size 和 stride 下，图像能划分成的块的数量（纵向和横向的块数）
        Ly = (h - kernel_size[0]) // stride[0] + 1
        Lx = (w - kernel_size[1]) // stride[1] + 1

        # 如果 uf 和 df 都是 1，不进行上采样或下采样
        if uf == 1 and df == 1:
            # 定义 unfold 和 fold 参数
            fold_params = dict(kernel_size=kernel_size, dilation=1, padding=0, stride=stride)  #创建了一个字典 fold_params  kernel_size:用于卷积操作的窗口大小；dilation:卷积核元素之间的间距（即扩张率）、
            #padding:输入图像的边缘添加的像素的数量； stride:卷积操作时在输入图像上移动的步幅。
            unfold = torch.nn.Unfold(**fold_params)  # 用于将图像展开
            fold = torch.nn.Fold(output_size=x.shape[2:], **fold_params)  # 主要设置一个合并器，用于将图像块重新折叠成完整图像

            # 计算归一化权重，用于规范重叠区域  weighting 张量将考虑到这些重叠，确保它们的信息在重建时得到合理加权
            weighting = self.get_weighting(kernel_size[0], kernel_size[1], Ly, Lx, x.device).to(x.dtype)  #to(x.dtype) 之后，weighting 张量的类型会被转换为与输入张量 x 相同的数据类型
            normalization = fold(weighting).view(1, 1, h, w)  # 对重叠区域进行规范化
            weighting = weighting.view((1, 1, kernel_size[0], kernel_size[1], Ly * Lx))  #Ly * Lx代表小块的数量
        ...
        # 如果有上采样需求 (uf > 1)
            ...
            # 定义 fold 参数，kernel_size 和 stride 根据上采样因子调整
            ...
            normalization = fold(weighting).view(1, 1, h * uf, w * uf)  # 对重叠区域进行规范化
        ...
        # 如果有下采样需求 (df > 1)
            ...
            # 定义 fold 参数，kernel_size 和 stride 根据下采样因子调整
            ...
            normalization = fold(weighting).view(1, 1, h // df, w // df)  # 对重叠区域进行规范化
            weighting = weighting.view((1, 1, kernel_size[0] // df, kernel_size[1] // df, Ly * Lx))

        # 如果上采样和下采样同时设置了，则抛出未实现异常
        else:
            raise NotImplementedError

        # 返回 fold 和 unfold 操作器、归一化张量、加权系数
    ...
    def decode_first_stage(self, z, predict_cids=False, force_not_quantize=False):
        """
        解码模型的第一阶段输出 z，可能包括对编码 ID 的预测和量化处理。

        :param z: 输入的编码张量，通常是通过某种模型编码生成。
        :param predict_cids: 是否预测编码 ID，默认为 False。
        :param force_not_quantize: 强制不进行量化处理，默认为 False。
        :return: 解码后的输出张量。
        """

        # 如果需要预测编码 ID
        if predict_cids:
            # 当 z 维度为 4 时，表示该输入可能为 batch size, channels, height, width
            if z.dim() == 4:
                # 通过指数运算计算概率，并选取最大概率的 ID
                z = torch.argmax(z.exp(), dim=1).long()
            # 从量化模型中获取对应的代码簿条目
            z = self.first_stage_model.quantize.get_codebook_entry(z, shape=None)
            # 调整张量的维度顺序，变为 (batch_size, channel, height, width)
            z = rearrange(z, 'b h w c -> b c h w').contiguous()

        # 应用比例因子，将 z 进行缩放
        z = 1. / self.scale_factor * z

        # 检查是否存在 `split_input_params` 属性
        if hasattr(self, "split_input_params"):
            # 如果允许分割输入的参数
            if self.split_input_params["patch_distributed_vq"]:
                ks = self.split_input_params["ks"]  # 获取卷积核大小
                stride = self.split_input_params["stride"]  # 获取步幅
                uf = self.split_input_params["vqf"]  # 获取上采样因子

                # 获取 z 的形状
                bs, nc, h, w = z.shape

                # 如果卷积核尺寸大于输入图像尺寸，进行调整
                if ks[0] > h or ks[1] > w:
                ...
                z = unfold(z)  # 将张量展开 (bn, nc * prod(ks), L)
                # 1. 重新调整为图像块形状
                z = z.view((z.shape[0], -1, ks[0], ks[1], z.shape[-1]))  # (bn, nc, ks[0], ks[1], L)

                # 2. 对每个小块应用模型，循环遍历最后一个维度
                ...
                # 将块合并回图像
                decoded = fold(o)
                decoded = decoded / normalization  # 进行归一化处理
    ...
    def encode_first_stage(self, x):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if self.split_input_params["patch_distributed_vq"]:
                ...
                ks = self.split_input_params["ks"]  # eg. (128, 128)
                stride = self.split_input_params["stride"]  # eg. (64, 64)
                df = self.split_input_params["vqf"]  # 向量量化因子

                # 保存输入图像的原始尺寸
                self.split_input_params['original_image_size'] = x.shape[-2:]
                bs, nc, h, w = x.shape  # 提取批量大小 (bs)，通道数 (nc)，高度 (h) 和宽度 (w)

                # 如果补丁尺寸大于图像的高度或宽度，调整补丁尺寸
                if ks[0] > h or ks[1] > w:
                    ks = (min(ks[0], h), min(ks[1], w))
                    print("reducing Kernel")
                ...
                # 对于每一个展开的补丁，通过 first_stage_model 进行编码
                output_list = [self.first_stage_model.encode(z[:, :, :, :, i])
                               for i in range(z.shape[-1])]

                # 将所有编码结果沿最后一个轴进行堆叠
                o = torch.stack(output_list, axis=-1)
                o = o * weighting  # 使用加权进行处理
                ...
                # 将补丁拼接回原始图像的结构
                decoded = fold(o)
                decoded = decoded / normalization  # 进行归一化
                return decoded  # 返回最终的解码输出

            else:
                # 如果没有进行补丁分布的向量量化，直接对 x 进行编码
                return self.first_stage_model.encode(x)
        else:
            # 如果没有分裂输入参数，直接对 x 进行编码
            return self.first_stage_model.encode(x)
...
class LatentDiffusionSRTextWTFFHQ(LatentDiffusionSRTextWT):
    # 覆盖父类的 get_input 方法，用于处理输入数据
    @torch.no_grad()  # 禁用梯度计算，减少内存消耗
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
                  cond_key=None, return_original_cond=False, bs=None, val=False, text_cond=[''], return_gt=False, resize_lq=True):

        # 将 batch 中的 'gt' (ground truth) 图像加载到 GPU 上
        im_gt = batch['gt'].cuda()
        # 将 ground truth 图像转换为 PyTorch 连续格式，并将数据类型转换为 float
        im_gt = im_gt.to(memory_format=torch.contiguous_format).float()

        # 将 batch 中的 'lq' (low quality) 图像加载到 GPU 上
        im_lq = batch['lq'].cuda()
        # 同样将低质量图像转换为连续格式和 float 类型
        im_lq = im_lq.to(memory_format=torch.contiguous_format).float()

        # 将低质量图像的值限制在 [0, 1.0] 范围内
        im_lq = torch.clamp(im_lq, 0, 1.0)  #.clamp(input, min, max) 是一个用于张量的操作函数，可以将张量中的每个元素限制在指定的最小值和最大值之间。

        # 保存 ground truth 和 low quality 图像
        self.gt = im_gt
        self.lq = im_lq

        # 如果需要调整低质量图像的尺寸，使用双三次插值（bicubic interpolation）
        if resize_lq:
            self.lq = F.interpolate(  #PyTorch 中用于图像重采样
                self.lq,
                size=(self.gt.size(-2), self.gt.size(-1)),  # 将lq图片的尺寸设置与gt一样大  -2为获取lq的高，-1为获取lq的宽
                mode='bicubic',
            )

        # 如果不处于验证模式且没有随机调整图像大小，则调用训练样本池方法（用于队列操作）
        if not val and not self.random_size:
            self._dequeue_and_enqueue()

        self.lq = self.lq.contiguous()  #连续分布

        # 将低质量和高质量图像的像素值从 [0, 1] 映射到 [-1, 1]，这在很多深度学习模型中是常见的预处理步骤
        self.lq = self.lq * 2 - 1.0
        self.gt = self.gt * 2 - 1.0

        # 如果使用随机尺寸，随机裁剪输入的 lq 和 gt
        if self.random_size:
            self.lq, self.gt = self.randn_cropinput(self.lq, self.gt)

        # 再次限制低质量图像的值范围到 [-1.0, 1.0]
        self.lq = torch.clamp(self.lq, -1.0, 1.0)

        # 有 0.5% 的概率将低质量图像设为 ground truth 图像，可能用于正则化或数据增强
        if random.random() < 0.005:
            self.lq = self.gt

        # 设置 x 为低质量图像，y 为 ground truth 图像
        x = self.lq
        y = self.gt

        # 如果指定了批大小（bs），则裁剪 x 和 y 的批大小
        if bs is not None:  #将 x 张量裁剪到前 bs 个元素
            x = x[:bs]
            y = y[:bs]

        # 将 x 和 y 移动到 GPU（device）上
        x = x.to(self.device)
        y = y.to(self.device)

        # 编码低质量图像 x，获取其潜在编码（latent code）
        encoder_posterior = self.encode_first_stage(x)
        z = self.get_first_stage_encoding(encoder_posterior).detach()  # 得到编码后的 latent 表示并分离计算图

        # 同样编码 ground truth 图像 y，获取其潜在编码
        encoder_posterior_y = self.encode_first_stage(y)
        z_gt = self.get_first_stage_encoding(encoder_posterior_y).detach()

        # 初始化用于位置编码的变量 xc
        xc = None
        # 如果使用位置编码，则计算位置偏移（未实现，抛出错误）
        if self.use_positional_encodings:
            assert NotImplementedError
            pos_x, pos_y = self.compute_latent_shifts(batch)
            c = {'pos_x': pos_x, 'pos_y': pos_y}

        # 确保 text_cond 的长度和 z 的 batch 大小一致，不够则扩展，超过则截取
        while len(text_cond) < z.size(0):
            text_cond.append(text_cond[-1])
        if len(text_cond) > z.size(0):
            text_cond = text_cond[:z.size(0)]
        ...
        # 输出包含潜在编码 z 和文本条件 text_cond
        out = [z, text_cond]
        out.append(z_gt)  # 还包含 ground truth 的潜在编码 z_gt

        # 如果需要返回第一阶段的输出，解码 z_gt 并将其加入输出
        if return_first_stage_outputs:
            xrec = self.decode_first_stage(z_gt)  # 解码潜在编码
            out.extend([x, self.gt, xrec])  # 加入原始输入和解码结果

        # 如果需要返回原始的条件编码，则加入 xc
        if return_original_cond:
            out.append(xc)
