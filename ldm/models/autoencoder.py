...
class AutoencoderKL(pl.LightningModule):
    def __init__(self,
        ...
        # 初始化参数
        self.image_key = image_key
        self.encoder = Encoder(**ddconfig)  # 初始化编码器
        self.decoder = Decoder(**ddconfig)  # 初始化解码器
        self.loss = instantiate_from_config(lossconfig)  # 实例化损失函数
        assert ddconfig["double_z"]  # 检查配置
    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=False):
        # 从给定路径加载模型检查点
        sd = torch.load(path, map_location="cpu")  # 加载检查点
        if "state_dict" in list(sd.keys()):
            sd = sd["state_dict"]  # 提取状态字典
    ...
    def encode(self, x, return_encfea=False):
        # 编码输入图像
        h = self.encoder(x)  # 通过编码器得到特征
        moments = self.quant_conv(h)  # 通过量化卷积层得到均值和方差
        posterior = DiagonalGaussianDistribution(moments)  # 创建高斯分布
        if return_encfea:
            return posterior, moments  # 返回分布和特征
        return posterior  # 返回仅分布

    def encode_gt(self, x, new_encoder):
        # 使用新的编码器进行编码
        h = new_encoder(x)  # 通过新编码器获取特征
        moments = self.quant_conv(h)  # 量化
        posterior = DiagonalGaussianDistribution(moments)  # 创建高斯分布
        return posterior, moments  # 返回分布和特征

    def decode(self, z):
        # 解码潜在表示
        z = self.post_quant_conv(z)  # 通过后量化卷积
        dec = self.decoder(z)  # 进行解码
        return dec  # 返回重构图像

    def forward(self, input, sample_posterior=True):
        # 执行前向传播
        posterior = self.encode(input)  # 编码输入
        if sample_posterior:
            z = posterior.sample()  # 从后验分布中采样
        else:
            z = posterior.mode()  # 取均值

        dec = self.decode(z)  # 解码潜在表示
        return dec, posterior  # 返回重构图像及后验分布
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        # 单个训练步骤
        inputs = self.get_input(batch, self.image_key)  # 获取输入
        reconstructions, posterior = self(inputs)  # 前向传播得到重构图像

        if optimizer_idx == 0:
            # 训练编码器+解码器+logvar
            aeloss, log_dict_ae = self.loss(inputs, reconstructions, posterior, optimizer_idx, self.global_step,
                                            last_layer=self.get_last_layer(), split="train")  # 计算损失
        ...
        if optimizer_idx == 1:
            ...
            return discloss  # 返回损失

    def validation_step(self, batch, batch_idx):
        # 验证步骤
        inputs = self.get_input(batch, self.image_key)  # 获取输入
        reconstructions, posterior = self(inputs)  # 前向传播
        ...
        # 记录验证损失
        self.log("val/rec_loss", log_dict_ae["val/rec_loss"])  # 重构损失
        self.log_dict(log_dict_ae)  # 其他日志
        self.log_dict(log_dict_disc)  # 判别器日志
        return self.log_dict  # 返回日志字典
    ...
    def configure_optimizers(self):
        ...
        opt_ae = torch.optim.Adam(list(self.encoder.parameters()) +
                                  ...
                                  list(self.quant_conv.parameters()) +
                                  list(self.post_quant_conv.parameters()),
                                  lr=lr, betas=(0.5, 0.9))  # 优化器用于编码器和解码器

        opt_disc = torch.optim.Adam(self.loss.discriminator.parameters(),
                                    lr=lr, betas=(0.5, 0.9))  # 优化器用于判别器
    ...
    def log_images(self, batch, only_inputs=False, **kwargs):
        # 日志记录图像
        log = dict()  # 初始化日志字典
        x = self.get_input(batch, self.image_key)  # 获取输入图像
        x = x.to(self.device)  # 转移到当前设备

        if not only_inputs:
            xrec, posterior = self(x)  # 记录重构图像
            if x.shape[1] > 3:
                # 如果有多于3个通道, 转换为RGB
                assert xrec.shape[1] > 3
                x = self.to_rgb(x)  # 将输入转换为RGB
                xrec = self.to_rgb(xrec)  # 将重构图像转换为RGB

            # log["samples"] = self.decode(torch.randn_like(posterior.sample()))  # 可选：记录采样的图像
            log["reconstructions"] = xrec  # 记录重构图像
    ...
    def to_rgb(self, x):
        # 将图像转换为RGB格式
        assert self.image_key == "segmentation"  # 确保处理的是分割图
        if not hasattr(self, "colorize"):
            self.register_buffer("colorize", torch.randn(3, x.shape[1], 1, 1).to(x))  # 初始化颜色映射
        x = F.conv2d(x, weight=self.colorize)  # 应用颜色映射
        x = 2. * (x - x.min()) / (x.max() - x.min()) - 1.  # 线性归一化到[-1, 1]
        return x  # 返回RGB图像
